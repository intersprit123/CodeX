...
const demoMarkets = [
  ['NIFTY 50','25,118.95','+0.82%'],
  ['SENSEX','82,365.77','+0.67%'],
  ['S&P 500','6,481.40','+0.21%'],
  ['NASDAQ','21,590.14','-0.18%'],
  ['NIKKEI','43,812.70','+1.10%'],
  ['DAX','24,921.10','+0.44%'],
]

const demoMovers = [
  ['Reliance Industries','₹1,418.30','+2.84%'],
  ['Tata Motors','₹684.75','+2.31%'],
  ['Infosys','₹1,492.20','+1.72%'],
  ['HDFC Bank','₹1,942.60','-0.84%'],
  ['ICICI Bank','₹1,431.40','+0.61%'],
]

const nav=[['Overview','/'],['Markets','/markets'],['Screener','/screener'],['Watchlist','/watchlist'],['Portfolio','/portfolio'],['News','/news'],['AI Research','/research'],['Alerts','/alerts'],['AI Copilot','/ai']]
...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    fetch('/api/market',{cache:'no-store'})
      .then(r=>r.ok?r.json():Promise.reject(new Error('market api failed')))
      .then(data=>{
        if(!active) return
        if(Array.isArray(data.quotes)&&data.quotes.length){
          setQuotes(data.quotes)
          setLive(Boolean(data.liveData))
        }
      })
      .catch(()=>{})
    return ()=>{active=false}
  ...
  const movers=useMemo(()=>{
    if(!quotes.length) return demoMovers
    return quotes.slice(0,5).map(q=>[
      q.name,
      new Intl.NumberFormat('en-IN',{style:'currency',currency:q.currency,maximumFractionDigits:2}).format(q.price),
      `${q.changePercent>=0?'+':''}${q.changePercent.toFixed(2)}%`
    ])
  },[quotes])

  const greeting = mounted && now ? getGreeting(now.getHours()) : 'MarketOS'
  const dateLabel = mounted && now ? formatOverviewDate(now) : 'MARKETOS'
  const timeLabel = mounted && now ? formatOverviewTime(now) : '--:--:--'
